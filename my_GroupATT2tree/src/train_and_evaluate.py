...
def generate_tree_input(target, decoder_output, nums_stack_batch, num_start, unk,generate_nums=2):
    ...
    for i in range(len(target)):
        if target[i] == unk:
            ...
            for num in num_stack:
                #num means the position of this number in num_list
                if decoder_output[i, num_start + generate_nums+num] > max_score:
                    target[i] = num + num_start+generate_nums
                    max_score = decoder_output[i, num_start+generate_nums + num]
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               ...
               merge_optimizer, output_lang, num_pos, english=False,current_batch_id=0):
    ...
    #    print("current batch group mask .size() : ",current_batch_group_mask.size())
    #    print("group mask [0] .size() : ",group)
    # Prepare input and output variables
