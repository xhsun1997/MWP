...
from groupATTEncoder import EncoderGroupATT


batch_size = 64
accumulation_gradient_step=2
embedding_size = 128
hidden_size = 512
n_epochs = 100
learning_rate = 5e-4
...
#train_data=load_json_data("./data/my_mwp_data.json")
...
print("．在input_lang中的id : ",input_lang.word2index["．"])
print("，在input_lang中的id : ",input_lang.word2index["，"])

#############################################定义模型的组成成分############################################################
encoder = EncoderGroupATT(input_lang=input_lang,input_size=input_lang.n_words, embedding_size=embedding_size, hidden_size=hidden_size,
                     n_layers=n_layers)
...
best_acc=0.0
writer=SummaryWriter(log_dir="./log_dir/plus_generate_nums/")
...
for epoch in range(n_epochs):
    ...
    for idx in range(len(input_lengths)):
        ...
        loss_total += loss
        if (idx+1)%accumulation_gradient_step==0:
    ...
    writer.add_scalar("loss",loss_total / len(input_lengths),epoch)
    writer.add_scalar("solution_acc",best_acc,epoch)
    print("Record loss and solution accuracy")
    ...
    if epoch % 10 == 0 or epoch > n_epochs - 5:
        ...
        if float(value_ac) / eval_total>best_acc:
            print("last epoch value acc is %f, current epoch value acc is %f "%(best_acc,float(value_ac) / eval_total))
            print("saved model to saved_models")
